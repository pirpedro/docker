...
# Check if a user and a group with respectively UID and GID exists.
if id -u ${USER_UID} > /dev/null 2>&1; then
    old_username=$(getent passwd "${USER_UID}" | cut -d: -f1)
    usermod -m -d /home/${USERNAME} -l ${USERNAME} ${old_username}
fi
...
MARKER_FILE="/usr/local/etc/app/common"
